...
module.exports = {
  ...
  allLogByProID: ProID =>
    db.load(`select * from bidding_log where ProID=${ProID}`),
  ...
  arrIdFromList: (UserID, table) =>
    db.load(`select DISTINCT  ProID from ${table} where UserID=${UserID}`),
  arrIdContinue: ProID => {
    var inlist = '';
  ...
  allContinue: () =>
    db.load(`select ProID from endtime where CURRENT_TIMESTAMP < endtime`),
  allMinuteAgo: minutes =>
    db.load(
      `select ProID from products where  (DATE_ADD(CreatedDTime, INTERVAL ${minutes} MINUTE)>CURRENT_TIMESTAMP)`
    ),
  allEnd: () =>
    db.load(`select ProID from endtime where CURRENT_TIMESTAMP > endtime`),
  ...
  isInBanBidList: (ProID, UserID) =>
    db.load(
      `select * from bannedlist where ProID=${ProID} and UserID=${UserID} `
    ),
  delBidLog: (ProID, UserID) => {
    console.log(` ${ProID} ${UserID} delBidLog `);
    db.del('bidding_log', { ProID: ProID, UserID: UserID });
  },
  loadWinWithPrice: ProID =>
    db.load(`select * from bidding_log where Price=(SELECT max(price) FROM bidding_log where  proid=${ProID})
    `),
  coutBid: ProID =>
    db.load(`select count(*) from products where ProID=${ProID}`),
  isEnd: proid =>
    db.load(
      `SELECT * FROM endtime where proid=${proid} and endtime< CURRENT_TIMESTAMP`
    ),
  topBidding: number =>
    db.load(`SELECT * FROM products order by NumberBid desc limit ${number}`),
  topPrice: number =>
    db.load(
      `select * from products where (ProID in (SELECT proid FROM endtime where endtime>CURRENT_TIMESTAMP)) order by PRICE desc limit ${number}`
    ),
  topEndtime: number =>
    db.load(
      `select * from products where proid in  (SELECT proid FROM endtime where endtime > current_timestamp order by endtime ) limit ${number}`
    )
};
